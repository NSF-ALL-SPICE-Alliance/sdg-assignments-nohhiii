...
```{r}
ggplot(data = sdr_data, aes(x= goal_13_score, y = goal_16_score, color = goal_9_score)) +
  geom_point()
```

```{r}
#INTERPRETATION: The scatter plot above measures the relationship between SDG goal 13 (Climate Action) and SDG goal 16 (Peace, justice and strong institutions), colored in a gradient by SDG goal 9 (Industry, innovation and infrastructure). Looking at the plot, it appears that the countries with the highest SDG goal 16 and SDG goal 9 scores (light blue) fall within the score range 50-75 for SDG goal 13. Meanwhile, countries that scored the highest for SDG 13 scores (80-100) had the lowest SDG goal 16 and SDG goal 9 scores (showed a declining pattern with higher SDG 13 points becoming darker and positioning lower for SDG 16). After viewing the SDG map, it appears that a lot of countries from the Sub-Saharan Africa region achieved SDG goal 13, but unfortunately still face challenges in SDG goal 16 and SDG goal 9.
```
...
```{r}
#INTERPRETATION: The histogram plot above measures various regions' compliance with the SDG goal 12 (Responsible Consumption and Production). From the data above, it appears that the regions of Sub-Sahara Africa and a few from South & East Asia scored the highest, while other countries such as the those belonging to the OECD-- though scattered from >40 to <90-- tended to hold some of the lowest scores. 
```
